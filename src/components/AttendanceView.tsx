            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-100">
              {isAdmin ? (
                <button
                  type="button"
                  onClick={() => {
                    const id = inspectPhotoModal.attendanceId;
                    const studentName = inspectPhotoModal.studentName;
                    setInspectPhotoModal(null);
                    handleDeleteAttendance(id, studentName);
                  }}
                  className="bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold px-4 py-2 rounded-xl text-xs border border-rose-200 flex items-center gap-1.5 cursor-pointer transition-all active:scale-95"
                >
                  <Trash2 className="w-4 h-4 text-rose-600" />
                  <span>Hapus Presensi Ini (Foto Fake/Salah)</span>
                </button>
              ) : <div />}
