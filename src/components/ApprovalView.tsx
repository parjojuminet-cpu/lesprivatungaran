  ...
  const handleProcess = async (id: string, status: 'Approved' | 'Rejected') => {
    ...
    try {
      await persistDatabaseUpdate(db => {
        ...
        if (status === 'Approved' && targetApp?.type === 'Pengajuan Kuota Extra' && targetApp.studentId) {
          const match = targetApp.reason.match(/\+(\d+)/);
          const addAmount = match ? parseInt(match[1], 10) : 4;
          updatedStudents = db.students.map(st => {
            if (st.id === targetApp.studentId) {
              const newRem = st.remainingSessions + addAmount;
              const newTotal = st.totalPackageSessions + addAmount;
              return {
                ...st,
        ...
        return { ...db, approvals: updatedApprovals, students: updatedStudents };
      });
      alert(`Pengajuan berhasil di-${status.toLowerCase()}${status === 'Approved' && targetApp?.type === 'Pengajuan Kuota Extra' ? ' & Kuota Extra Murid telah ditambahkan!' : ''}`);
      onRefresh();
    } catch (err) {
