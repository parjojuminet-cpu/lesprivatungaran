          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200">
              <tr>
                ...
                <th className="p-3.5 text-right">Honor / Sesi (Rp)</th>
                {isManagement && <th className="p-3.5 text-center">Aksi Manajemen</th>}
              </tr>
            </thead>
                ...
                filteredSchedules.map((sch) => {
                  ...
                  return (
                    <tr key={sch.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      {/* Honor / Sesi */}
                      <td className="p-3.5 text-right whitespace-nowrap font-extrabold text-emerald-700">
                        {formatRupiah(sch.sessionRate || tutor?.ratePerSession || 40000)}
                        <span className="text-[10px] text-slate-400 font-normal block">/ sesi</span>
                      </td>
                        ...
                        <td className="p-3.5 text-center whitespace-nowrap">
                          <button
                            onClick={() => setScheduleToDelete({
                              id: sch.id,
                              studentName: student?.name || 'Siswa N/A',
                              day: sch.dayOfWeek,
                              time: sch.timeSlot,
                              subject: sch.subject,
                              tutorName: tutor?.name || 'Tentor N/A'
                            })}
                            className="bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 font-bold px-3 py-1.5 rounded-xl text-xs inline-flex items-center gap-1 cursor-pointer transition-all active:scale-95"
                            title="Hapus Alokasi Jadwal Ini (TIDAK menghapus data murid)"
                          >
