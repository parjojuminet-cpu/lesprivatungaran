...
export interface Schedule {
  ...
  type: ScheduleType;
  sessionRate?: number; // HONOR TENTOR PER SESI (TARIF SPP SISWA - FEE ADMIN)
  adminFee?: number; // FEE ADMIN / POTONGAN BIMBEL (MISAL 5000, 7000, DLL)
...
export interface Invoice {
  ...
  studentId: string;
  amount: number; // Total Tagihan = (sessionCount * ratePerSession) + additionalAmount
  amountPaid: number;
  status: InvoiceStatus;
  dueDate: string;
  createdAt: string;
  sessionCount?: number;       // Sesi / token terpakai (ditarik otomatis)
  ratePerSession?: number;     // Tarif SPP per sesi
  additionalAmount?: number;   // Nominal Tambahan (Denda keterlambatan / Perlengkapan tentor)
  additionalNotes?: string;    // Keterangan cas tambahan
  additionalTutorId?: string;  // ID Tentor yang mengajukan/terkait cas tambahan
  isRevised?: boolean;
  revisedAt?: string;
  revisionNote?: string;
}
